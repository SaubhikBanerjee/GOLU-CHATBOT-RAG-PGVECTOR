...
def simple_vector_query(query_string):
...
def keyword_vector_query(query_string):
    model = SentenceTransformer("sentence-transformers/msmarco-distilbert-base-v4")
    conn = None
    try:
        ...
        print("Connected to: ", db_version)
        # Now getting the query embeddings for the query text.
        query_text = ' '.join(keywords(query_string))
        query_embeddings = model.encode(query_text)
        sql_text = "SELECT sectionid, sectiontext FROM chunk " \
                   "WHERE 1=1  " \
                   " and 1=1 " \
...
if __name__ == '__main__':
    ...
    simple_vector_query(query_text)
    print("\033[38;5;208m====================================\033[0;0m")
    keyword_vector_query(query_text)
